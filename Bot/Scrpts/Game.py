...
from game_db import init_db, create_game, get_game_by_chat_id, get_game_by_master_id, get_all_enemies, add_player
...
async def help(message: types.Message):
    await message.answer(
        ...
        "/roll [d(любое число)] - позволяет кинуть кубик на четное d\n"
    ...
    if await get_game_by_master_id(message.from_user.id):
        try:
            await message.bot.send_message(
                ...
                text="Вы являетесь мастером в активной игре, закройте игру, чтобы стать открыть новую!",
                parse_mode="Markdown"
            )
...
async def callback_procced(callback: types.CallbackQuery):
    ...
    if await get_game_by_chat_id(callback.message.chat.id):
        ...
        await callback.answer(
                "У чата нет активной игры!",
                parse_mode="Markdown",
            )
        return

    if await get_game_by_master_id(callback.from_user.id):
        try:
            await callback.bot.send_message(
                chat_id=callback.from_user.id,
                text="Вы являетесь мастером в активной игре, закройте игру, чтобы стать игроком!",
                parse_mode="Markdown"
            )
        except Exception as e:
            print(f"Не удалось отправить личное сообщение: {e}")
    ...
    await add_player(game_id, callback.from_user.id, callback.from_user.username, 0, {})
