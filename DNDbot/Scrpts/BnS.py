...
async def help(message: types.Message):
    await message.answer(
        "Вот более подробный гайд для котроля игр: \n\n",

        "Для Мастеров:\n",
        "/delete_game - удаляет игру, где вы мастер\n",
        "/add_en [Имя] [Хп] [Видимость хп Врага, 1 или 0] [Видимость Врага, 1 или 0] - добовляет врага\n",
        ...
        "Для игрока:\n"
        "/asl [STR] [INT]... - ( add stat list ) добовляет вам статы"
    )
...
async def delete_game(message: types.Message, command: CommandObject):
    if not command.args:
        ...
        if await get_game_by_master_id(message.from_user.id):
            ...
            await message.answer(
                "Укажите ID игры для удаления! Пример: `/delete_game GAME123`",
                parse_mode="Markdown",
            )
        return
...
async def add_enemy(message: types.Message, command: CommandObject):
    if not command.args:
        await message.answer(
...
async def update_enemy(message: types.Message, command: CommandObject):
...
async def show_enemies(message: types.Message, command: CommandObject):
...
async def show_all_players_handler(message: types.Message):
